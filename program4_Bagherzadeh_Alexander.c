...
//Add trees
node_t * addTree(node_t * t1, node_t * t2) {
    if((t1 != NULL) && (t2 != NULL)) { //If both nodes are not NULL, then recursively call the children and apply the addition
        t1->leftchild = addTree(t1->leftchild, t2->leftchild);
        t1->rightchild = addTree(t1->rightchild, t2->rightchild);
        t1->data = t1->data + t2->data;
        return t1;
    } else if (t2 != NULL) { //If tree 2's node is not NULL, return that node to the tree(t1)
        return t2;
    } else { //Last case which is doing nothing, just returning t1's node to t1
        return t1;
    }
}
...
//Sub trees
node_t * subtractTree(node_t * t1, node_t * t2) {
    if((t1 != NULL) && (t2 != NULL)) { //If both nodes are not NULL, then recursively call the children and apply the subtraction
